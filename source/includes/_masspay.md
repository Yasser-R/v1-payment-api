...
MassPay offers a significant advantage over repeatedly calling the [Send](#send-money) endpoint for each individual transaction, which is the fact that a bank-funded MassPay Job only incurs a single ACH transaction to fund the entire batch of payments.  The alternative approach would incur an ACH transaction from the bank funding source for each individual payment.  Those who used this approach have reported incurring fees from their financial institutions for excessive ACH transactions.
